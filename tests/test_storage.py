...
def test_storage_keeps_turns_and_latest_progress(tmp_path: Path) -> None:
    ...
    session = storage.create_session("006", "bank:agent:006", "deep", "source-hash", "原题")
    ...
    assert saved.knowledge_id == "bank:agent:006"


def test_storage_lists_newest_sessions_first(tmp_path: Path) -> None:
    storage = SQLiteStorage(tmp_path / "practice.db")
    first = storage.create_session("006", "bank:agent:006", "single", "a", "第一题")
    second = storage.create_session("022", "bank:tool:022", "deep", "b", "第二题")
